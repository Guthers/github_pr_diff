...
(() => {
        ...
        for (const link of allCommitLinks) {
            ...
            if (sha) {
                if (!seenSHAs.has(sha)) {
                    ...
                    if (inCommitsList) {
                        commitsInTimeline.push({ sha, element: link });
                    }
                }
            }
        }
    ...
    const createDiffIcon = (commitSHA, latestSHA, prInfo) => {
