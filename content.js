...
(function() {
  ...
  function extractSHAFromHref(href) {
    // Match patterns like:
    // - /commit/SHA (singular)
  ...
  function getLatestCommitSHA() {
    ...
    const prHeader = document.querySelector('.gh-header-meta, .commit-tease, [data-test-selector="pr-header-actions"], .gh-header-actions');
    if (prHeader) {
      const headerLinks = prHeader.querySelectorAll('a[href*="/commit"]');
    ...
    for (const link of allCommitLinks) {
      ...
      if (sha) {
        if (!seenSHAs.has(sha)) {
          ...
          const inCommitsList = link.closest('.TimelineItem, .commit-group, [data-test-selector="pr-timeline-commits-list"], .js-navigation-item, [data-testid="pr-timeline-commits-list"]');
          if (inCommitsList) {
            commitsInTimeline.push({ sha, link });
          }
        }
      }
    }
    ...
    if (allCommitLinks.length > 0) {
      ...
      const commitsSection = document.querySelector('[data-testid="pr-timeline-commits-list"], .js-navigation-container, .TimelineItem--condensed');
      if (commitsSection) {
        const sectionLinks = commitsSection.querySelectorAll('a[href*="/commit"]');
  ...
  function isCommitHashLink(link) {
  ...
  function findCommitHashes() {
    const commitHashes = new Map(); // Map SHA to element for deduplication
    const elements = [];
    ...
    allCommitLinks.forEach(link => {
      ...
      if (sha && isCommitHashLink(link)) {
        ...
        if (!commitHashes.has(sha) || commitHashes.get(sha) !== link) {
          commitHashes.set(sha, link);
          elements.push({ sha, element: link });
        }
      }
    });
  ...
  function createDiffIcon(commitSHA, latestSHA, prInfo) {
